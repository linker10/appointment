...
from club.models import ReservationDate
...
class ReservationTimeView(DetailView, FormView):
    ...
    def form_valid(self, form):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"UPDATE club_ReservationDate SET {person_info['Time_Select']} = {reservation_id} WHERE day_id = {self.kwargs['pk']}")
...
class ReservationCancelView(TemplateView, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ids_info = \
            PersonInfo.objects.filter(reservation_code=info['your_code'], email=info['your_email']).values('date_id',
                                                                                                           'reservation_id')[
                0]  # get date info
        except:
            print(form)
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"UPDATE club_ReservationDate SET {time_to_change} = 'open' WHERE day_id = {ids_info['date_id']}")  # set default value

            cursor.execute(
                f"DELETE FROM club_PersonInfo WHERE reservation_id = '{ids_info['reservation_id']}'")  # delete person
